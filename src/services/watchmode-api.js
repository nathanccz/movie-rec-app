...
export const getStreamSources = async (srcId) => {
    try {
        const WATCHMODE_API_KEY = import.meta.env.VITE_WATCHMODE_KEY
        const STREAM_SRC_URL = `https://api.watchmode.com/v1/sources/?apiKey=${WATCHMODE_API_KEY}`
        const response = await fetch(STREAM_SRC_URL)
        const data = await response.json()
