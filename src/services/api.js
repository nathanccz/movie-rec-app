...
export const addMovie = async (body) => {
  try {
    const response = await fetch(
      `http://localhost:3000/api/movies/${body.tmdbId}/add`,
      {
        method: 'POST',
...
export const getRecommendations = async (userID) => {
  try {
    const response = await fetch(
      `http://localhost:3000/api/movies/${userID}/recommendations`,
      {
        credentials: 'include',
      }
    )
...
export const getOpenAIRecommendations = async (mediaType, title) => {
  try {
    const URL = 'http://localhost:3000/api/movies/openai'
    const response = await fetch(URL, {
      method: 'POST',
      headers: {
